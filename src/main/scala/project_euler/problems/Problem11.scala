...
object Problem11 {
  ...
  def diag(list: List[List[Int]]): List[List[Int]] = {
    val dimension = list(0).length
    Stream.range(0, dimension).map(x =>
      Stream.range(0, dimension - x).map(i => list(i + x)(i)).toList
    ).toList ++ Stream.range(0, dimension).map(x =>
      Stream.range(0, dimension - x).map(i => list(i)(i + x)).toList
    ).toList
  }

  def diagreverse(list: List[List[Int]]): List[List[Int]] = {
    val dimension = list(0).length
    Stream.range(0, dimension).map(x =>
      Stream.range(0, x).map(i => list(x - i)(i)).toList
    ).toList ++ Stream.range(0, dimension).map(x =>
      Stream.range(0, x).map(i => list(i)(x - i)).toList
    ).toList
  }

  def windowedApply(list: List[Int], window: Int, index: Int = 0): List[Int] = {
    val selector = (a: List[Int], b: List[Int]) => a.product match {
      case g: Int if g > b.product => a
      case g: Int if g < b.product => b
      case g: Int => a
    }
    index match {
      case g: Int if g >= list.length - window => List.empty
      case g: Int => selector(Stream.range(g, g + window).map(p => list(p)).toList, windowedApply(list, window, g + 1))
    }
  }

  def main(args: Array[String]): Unit = {
    val horizontal = values.flatMap(_.toBuffer)
    assert(horizontal.length == 20 * 20)
    val vertical = Stream.range(0, 20).flatMap(i => horizontal.grouped(20).map(l => l(i))).toList
    assert(vertical.length == 20 * 20)
    val diagonals = diag(values) ++ diagreverse(values)
    assert(diagonals.length == 20 + 20 + 20 + 20)
    println(s"Horizontal: $horizontal")
    println(s"Vertical: $vertical")
    println(s"Diagonal: $diagonals")
    val horizontalProduct = windowedApply(horizontal, 4).product
    val verticalProduct = windowedApply(vertical, 4).product
    val diagonalProduct = diagonals.map(diag => windowedApply(diag, 4).product).toList
    val max = (List(horizontalProduct) ++ List(verticalProduct) ++ diagonalProduct).max
    println(s"$max $horizontalProduct $verticalProduct $diagonalProduct")
  }
}
